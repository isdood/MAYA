...
test "PatternTransformCache basic scaling" {
    ...
    const pattern = try ctx.cache.createPattern(&pattern_data, 2, 2);
    defer {
        pattern.deinit(ctx.allocator);
        ctx.allocator.destroy(pattern);
    }
...
test "PatternTransformCache LRU behavior" {
    ...
    const pattern = try PatternTransform.Pattern.create(testing.allocator, &pattern_data, 2, 2);
    defer {
