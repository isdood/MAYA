...
pub const Buffer_legacy = struct {
    ...
    fn findMemoryType(
        ...
        std.debug.print("    Type filter: {b}\n", .{type_filter});
        std.debug.print("    Required properties: {b}\n", .{properties});
        ...
        for (0..mem_properties.memoryTypeCount) |i| {
            ...
            const has_type = (type_filter & (@as(u32, 1) << @intCast(i))) != 0;
            const has_props = (memory_type.propertyFlags & properties) == properties;
            
            std.debug.print("    [{}] Type: {b}, Properties: {b} {s}{s}\n", .{
                i,
                @as(u32, 1) << @intCast(i),
                memory_type.propertyFlags,
                if (has_type) "[TYPE_MATCH]" else "",
                if (has_props) "[PROPS_MATCH]" else "",
            });
