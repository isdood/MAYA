// This is an enhanced version of the memory management example with better error handling and debug output
...
const VulkanContext = struct {
    ...
    pub fn init(allocator: std.mem.Allocator) !@This() {
        ...
        var instance: c.VkInstance = undefined;
        {
            ...
            std.debug.print("  create_info.sType = {}\n", .{create_info.sType});
            std.debug.print("  create_info.pApplicationInfo = {*}\n", .{create_info.pApplicationInfo});
            
            if (create_info.pApplicationInfo) |app_info_ptr| {
                std.debug.print("    app_info.sType = {}\n", .{app_info_ptr.sType});
                std.debug.print("    app_info.pApplicationName = {s}\n", .{app_info_ptr.pApplicationName});
            } else {
                std.debug.print("  Warning: pApplicationInfo is null\n", .{});
            }
            ...
            if (result != c.VK_SUCCESS) {
                ...
                if (result == c.VK_ERROR_EXTENSION_NOT_PRESENT) {
                    std.debug.print("  Error: Required extension not present\n", .{});
                } else if (result == c.VK_ERROR_LAYER_NOT_PRESENT) {
                    std.debug.print("  Error: Required layer not present\n", .{});
                } else if (result == c.VK_ERROR_INCOMPATIBLE_DRIVER) {
                    std.debug.print("  Error: Incompatible Vulkan driver\n", .{});
                    std.debug.print("  Please ensure you have a Vulkan-capable GPU and drivers installed\n", .{});
        ...
        if (!device_found) {
            std.debug.print("  No suitable physical device found\n", .{});
...
pub fn main() !void {
    ...
    var memory_manager = MemoryManager.init(context.device, context.physical_device, allocator);
...
test "memory management example" {
    ...
    _ = @import("memory_management_enhanced.zig");
}
