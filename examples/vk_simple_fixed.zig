...
pub fn main() !void {
    std.debug.print("Starting Vulkan simple example...\n", .{});
    ...
    const app_info = std.mem.zeroInit(c.VkApplicationInfo, .{
        ...
        .pNext = null,
        .pApplicationName = "Vulkan Simple Example",
    ...
    const cleanup = struct {
        fn cleanup(instance: VkInstance, vkGetInstanceProcAddr: *const FnVkGetInstanceProcAddr) void {
            const vkDestroyInstance_ptr = vkGetInstanceProcAddr(instance, "vkDestroyInstance");
    ...
    // Load a device-level function to demonstrate function loading
    ...
    std.debug.print("Vulkan simple example completed successfully!\n", .{});
}
