...
const pattern_matching = @import("../src/neural/pattern_matching.zig");
...
pub fn main() !void {
    ...
    for (0..height) |y| {
        for (0..width) |x| {
            const noise = 0.1 * (rng.random().float(f32) - 0.5);
    ...
    // Calculate the expected position (center of the pattern)
    const expected_x = offset_x + pattern_size / 2;
    const expected_y = offset_y + pattern_size / 2;
