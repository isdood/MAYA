// This is a working version of the memory management example with proper debug prints
...
const VulkanContext = struct {
    ...
    pub fn init(allocator: std.mem.Allocator) !@This() {
        ...
        var physical_device: ?c.VkPhysicalDevice = null;
        {
            ...
            std.debug.print("  Selected physical device: 0x{x}\n", .{@intFromPtr(physical_device)});
        }
        ...
        if (physical_device) |pd| {
            ...
            if (result == c.VK_SUCCESS) {
                device = dev;
                std.debug.print("  Logical device created successfully: 0x{x}\n", .{@intFromPtr(dev)});
            } else {
...
const MemoryManager = struct {
    ...
    pub fn createBuffer(self: *@This(), size: c.VkDeviceSize, usage: c.VkBufferUsageFlags, properties: c.VkMemoryPropertyFlags) !struct {
        ...
        std.debug.print("    Buffer created: 0x{x}\n", .{@intFromPtr(buffer)});
        ...
        std.debug.print("    Memory allocated: 0x{x}\n", .{@intFromPtr(buffer_memory)});
...
pub fn main() !void {
    ...
    std.debug.print("  vk_buffer: 0x{x}\n", .{@intFromPtr(buffer.buffer)});
    std.debug.print("  memory: 0x{x}\n", .{@intFromPtr(buffer.memory)});
...
test "memory management example" {
    ...
    _ = @import("memory_management_working.zig");
}
