...
pub fn main() !void {
    ...
    physical_device.getMemoryProperties(&memory_props);
    
    const memory_type_index = blk: {
        const mem_flags = vk.MemoryPropertyFlags{ .host_visible_bit = true, .host_coherent_bit = true };
        ...
        for (0..memory_props.memory_type_count) |i| {
            ...
            const memory_type = memory_props.memory_types[i];
            const has_required_flags = memory_type.property_flags.contains(mem_flags);
