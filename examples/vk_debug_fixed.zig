...
fn loadVulkan() !void {
    ...
    const vkGetInstanceProcAddr: *const fn (instance: VkInstance, pName: [*:0]const u8) callconv(.C) ?*const anyopaque = @ptrCast(c.dlsym(handle, "vkGetInstanceProcAddr"));
    ...
    ) callconv(.C) VkResult = @ptrCast(@alignCast(vkGetInstanceProcAddr(null, "vkCreateInstance") orelse {
    ...
    if (result == c.VK_SUCCESS) {
        ...
        const vkDestroyInstance: *const fn (instance: VkInstance, pAllocator: ?*const anyopaque) callconv(.C) void = @ptrCast(@alignCast(
            vkGetInstanceProcAddr(instance, "vkDestroyInstance") orelse {
